...
public class ManageStudentsFrame extends JFrame {
    ...
    private void export(){JFileChooser c=new JFileChooser();c.setSelectedFile(new File("students.csv"));if(c.showSaveDialog(this)==JFileChooser.APPROVE_OPTION)try{ExcelHelper.exportStudents(c.getSelectedFile().toPath(),current);JOptionPane.showMessageDialog(this,"Export complete.");}catch(Exception e){error(e);}}
    private void importCsv(){JFileChooser c=new JFileChooser();if(c.showOpenDialog(this)!=JFileChooser.APPROVE_OPTION)return;try{int count=0;for(Student s:ExcelHelper.importStudents(c.getSelectedFile().toPath())){String issue=Validator.validateStudent(s);if(issue!=null)throw new IllegalArgumentException(s.rollNumber()+": "+issue);db.saveStudent(s);count++;}refresh();JOptionPane.showMessageDialog(this,count+" student(s) imported.");}catch(Exception e){error(e);}}
